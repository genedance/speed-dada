#!/usr/bin/env python3
"""Four-way comparison: R dada2, SpeedDada, Python dada2, rust-only CLI."""
import json
import math
import sys
from pathlib import Path

BASE = Path("/tmp/bench_jhi_out")
TOOLS = [
    ...
    ("rust-only",    BASE / "rust_only" / "rust_only_output.json", BASE / "rust_only" / "rss.txt"),
]
...
def main():
    ...
    print()
    print("=" * 100)
    print("  RESOURCE USE  (peak RSS via /usr/bin/time -l)")
    for name, _, _, rss in results:
        print(f"  {name:<16}  {fmt_mb(rss)}")

    # Speedup vs R dada2 (if present)
    r_ref = next((r for r in results if r[0] == "R dada2"), None)
    if r_ref:
        ref_total = r_ref[1].get("total_ms")
        print()
        print("=" * 100)
        ...
        for name, data, _, _ in results:
            ...
            if ref_total and total:
                print(f"  {name:<16}  {ref_total / total:>7.2f}×")
