#!/usr/bin/env Rscript
# Benchmark: original R dada2 on 3 paired JHI samples.
...
out_dir   <- if (length(args) >= 3) args[3] else "/tmp/bench_jhi_out/r"
...
cat("[learnErrors]\n")
t <- proc.time()
errF <- learnErrors(fwd_filt, multithread = n_threads, verbose = FALSE)
errR <- learnErrors(rev_filt, multithread = n_threads, verbose = FALSE)
