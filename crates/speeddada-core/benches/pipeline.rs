...
fn make_fastq_records(
    n: usize,
    seq_len: usize,
    error_rate: f64,
    seed: u64,
) -> Vec<speeddada_core::io::fastq::FastqRecord> {
    let mut s = seed;
    ...
    (0..n)
        .map(|i| {
            ...
            speeddada_core::io::fastq::FastqRecord {
                id: format!("r{i}"),
                seq,
                qual,
            }
...
fn bench_taxonomy_classify(c: &mut Criterion) {
    ...
    for n_queries in [10usize, 50, 200] {
        ...
        g.bench_with_input(
            BenchmarkId::new("n_queries", n_queries),
            &n_queries,
            |b, _| {
...
fn bench_dada(c: &mut Criterion) {
    ...
    for n_reads in [200usize, 500, 1000] {
        ...
        let error_model =
            learn_errors(&records, &em_cfg).unwrap_or_else(|_| ErrorModel::illumina_default());
        let uniques: Vec<UniqueSeq> = {
            ...
            for r in &records {
                let e = map
                    .entry(r.seq.clone())
                    .or_insert_with(|| (0, vec![0.0; r.seq.len()]));
                e.0 += 1;
