"""Smoke tests for the speeddada Python bindings."""
...
def test_filter_and_trim():
    ...
    try:
        cfg = speeddada.FilterConfig(trunc_len=30, min_len=10, max_ee=10.0)
        stats = speeddada.filter_and_trim(cfg, str(SAMPLE_FASTQ), out_path)
...
def test_learn_errors():
    model = speeddada.learn_errors([str(SAMPLE_FASTQ)], n_reads=100)
    assert model is not None


def test_run_dada_remove_bimeras_roundtrip():
    derep = speeddada.derep_fastq(str(SAMPLE_FASTQ))
    assert len(derep) > 0

    model = speeddada.learn_errors([str(SAMPLE_FASTQ)], n_reads=100)
    result = speeddada.dada(derep, model, omega_a=1e-5)
    assert len(result) > 0

    seqs_with_abund = [(result[i][0], result[i][1]) for i in range(len(result))]
    clean = speeddada.remove_bimera_denovo(seqs_with_abund)
    assert len(clean) > 0
